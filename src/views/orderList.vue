<template>
  <div
      class="nav-wrapper orderList position-relative end-0 py-3 px-4 mx-4"
      style="direction: rtl"
  >
    <div class="menu-list">
        ...
        <div v-if="orderList.length" class="table-body">
          ...
          >
            <ul :id="'styleRow' + i" class="styleRow">
              ...
              <li>{{ userState == 0 ? item.sellerBrand : userState == 2 ? item.name : item.name + `(${item.sellerBrand})` }}</li>
              <li>{{ item.orderId }}</li>
                  ...
                  <div class="address">
                    ...
                    <div class="btn-factor">
                      <button
                          class="show-map"
                          @click="
												getMap(item.latitude, item.longitude, item.address)
											"
                      >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
        ...
        <div v-if="orderList.length">
          <div class="info-item" v-for="(item, i) in orderList" :key="i">
            ...
            <div class="item">
              ...
              <p class="item-data">{{ userState == 0 ? item.sellerBrand : userState == 2 ? item.name : item.name + `(${item.sellerBrand})` }}</p>
            </div>
            <div class="item">
              ...
              <div class="collapse" :id="'col' + i">
                <div class="card-body card-factor">
                  ...
                  <div class="item">
                    ...
                    <p class="item-data">{{ item.deliveryType }}</p>
                  </div>
                  <div class="item">
...
<!--        ></iframe>-->
          ...
          <div class="table-modal">
            <table class="table table-bordered">
              ...
              <tr>
                ...
                <td class="text-center">{{ modalDataFactor.orderId }}</td>
    ...
    <table class="d-none">
      ...
      <tbody v-for="row in orderList" :key="row">
...
export default {
  ...
  methods: {
    ...
    getOrderList() {
      ...
      this.$store
          .dispatch("getOrderListMain", data)
          .then((respons) => {
            this.isLoading = false;
            if (respons.data.succeed) {
              this.isSearchLoading = false;
              this.orderList = respons.data.orders;
              this.orderList.forEach((item) => {
                ...
                item.items.forEach((text, index) => {
                  ...
                  text.totalPriceWithDiscount = (text.totalPriceWithDiscount * 10).toString()
                      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                  // item.productNameCsv = text.productName;
                  // item.priceCsv = text.price;
                  // item.orderCountCsv = text.orderCount;
                  // item.totalPriceRowCsv = text.totalPrice;
                  // item.totalPriceCsv = (item.productPrice * 10).toString()
                  //     .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                  // item.sellerDiscountCsv = text.sellerDiscount;
                  // item.totalPriceWithDiscountCsv = text.totalPriceWithDiscount;
                });
              })
...
.orderList {
  ...
  .select-filter {
    display: grid;
    gap: 13px;
    grid-template-columns: 13% 16% 12% 12% 1fr;
...
.table-custom {
  ...
  ul {
    display: grid;
    grid-template-columns: 3% 20% 10% 15% 11% 11% 10% 15% 5%;
    width: 100%;
    align-items: baseline;
    justify-items: center;
  }
