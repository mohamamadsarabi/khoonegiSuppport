      ...
      <div v-else-if="windowWith <= 768" class="table-info-mobile">
        <div v-if="reportList.length">
          <div class="info-item" v-for="(item, i) in reportList" :key="i">
            ...
            <div class="item">
              <span class="item-head">اعتبار ابتدای دوره</span>
              <p class="item-data">{{ (item.beginningCredit * 10) }}</p>
            </div>
            <div class="item">
              <span class="item-head">افزایش اعتبار</span>
              <p class="item-data">
                {{ (item.increaseCredit * 10) }}
              </p>
            </div>
<!--            <div class="item">-->
<!--              <span class="item-head">افزایش اعتبار آنلاین</span>-->
<!--              <p class="item-data">{{ item.onlineCharging }}</p>-->
<!--            </div>-->
            <div class="item">
              <span class="item-head">کاهش اعتبار</span>
              <p class="item-data">
                {{ (item.creditReduction * 10) }}
              </p>
            </div>
<!--            <div class="item">-->
<!--              <span class="item-head">واریز آنلاین</span>-->
<!--              <p class="item-data">-->
<!--                {{ item.buyOnline }}-->
<!--              </p>-->
<!--            </div>-->
            <div class="item">
              <span class="item-head">اعتبار پایان دوره</span>
              <p class="item-data">
                {{ (item.endCredit * 10) }}
              </p>
            </div>
            <div class="item">
              <span class="item-head">مجموع خرید</span>
              <p class="item-data">
                {{ (item.totalPurchase * 10) }}
              </p>
            </div>
            <div class="item">
              <span class="item-head">اعتبار جاری</span>
              <p class="item-data">
                {{ (item.currentCredit * 10) }}
              </p>
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          label: "اعتبار ابتدای دوره",
          field: "beginningCredit",
          width: ".2%",
          sortable: false,
        },
        {
        ...
        // {
        //   label: "افزایش اعتبار آنلاین",
        //   field: "onlineCharging",
        //   width: ".3%",
        //   sortable: false,
        // },
        {
          label: "کاهش اعتبار",
          field: "creditReduction",
          width: ".1%",
          sortable: false,
        },
        // {
  ...
  methods: {
    ...
    getReportList() {
      ...
      this.$store
          .dispatch("creditReportCustomer", data)
          .then((respons) => {
            console.log(respons);
            this.isLoading = false;
            this.isSearchLoading = false;
            if (respons.data.succeed) {
              this.reportList = respons.data.creditReport;
              this.reportList.forEach((item) => {
                item.beginningCredit = (item.beginningCredit * 10).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                item.buyOnline = item.buyOnline.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                item.creditReduction = (item.creditReduction * 10).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                item.currentCredit = (item.currentCredit * 10).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                item.endCredit = (item.endCredit * 10).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                item.increaseCredit = (item.increaseCredit * 10).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                item.onlineCharging = item.onlineCharging.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                item.totalPurchase = (item.totalPurchase * 10).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
              })
            } else {
