<template>
  <div
      class="nav-wrapper ComprehensiveProduction position-relative end-0 py-3 px-4 mx-4"
      style="direction: rtl"
  >
      ...
      <div v-else-if="windowWith <= 768" class="table-info-mobile">
        <div v-if="reportList.length">
          <div class="info-item" v-for="(item, i) in reportList" :key="i">
            ...
            <div class="item">
              <span class="item-head">تعداد محصول تایید شده</span>
              <p class="item-data">
                {{ item.approvedProductsCount }}
              </p>
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          label: "تعداد محصول تایید شده",
          field: "approvedProductsCount",
          width: ".3%",
          sortable: false,
        },
      ],
  ...
  methods: {
    ...
    getReportList() {
      ...
      this.$store
          .dispatch("ComprehensiveProduction", data)
          .then((respons) => {
            console.log(respons);
            this.isLoading = false;
            if (respons.data.succeed) {
              this.reportList = respons.data.approvedProducts;
