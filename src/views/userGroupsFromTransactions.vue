<template>
  <div
      class="nav-wrapper userGroup userList position-relative end-0 py-3 px-4 mx-4"
      style="direction: rtl"
      ...
      <div class="table-info pt-3" v-if="windowWith > 768">
        <table class="khoonegi-table">
          ...
          <tr v-for="(row , index) in userList" :key="index">
            ...
            <td>
              <button @click="removeUser(row.customerId)">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <mask id="mask0_3075_3137" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="24"
                        height="24">
      ...
      <div v-else-if="windowWith <= 768" class="table-info-mobile">
        <div v-if="userList.length">
          <div class="info-item" v-for="(item, i) in userList" :key="i">
            ...
            <div class="item">
              <span class="item-head">تاریخ عضویت در گروه</span>
              <p class="item-data">{{ item.registerDate }}</p>
            </div>
            <div class="item">
              <span class="item-head">حذف نفر</span>
              <p class="item-data">
                <button @click="removeUser(item.customerId)">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <mask id="mask0_3075_3137" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="24"
                          height="24">
                      <rect width="24" height="24" fill="#D9D9D9"/>
  ...
  <Dialog v-model:visible="newUser" class="user-modal">
    <template #header>
      <div class="header">
        <h3>افزودن عضو جدید به گروه {{ groupName }}</h3>
        <button @click="newUser = false">✘</button>
      </div>
    </template>
    <div class="content">
      <h6 class="title-dialog">لطفا شماره موبایل یا نام و نام خانوادگی عضو جدید را وارد نمایید</h6>
      <div class="search-box search-customer">
        <vsud-input
            :value="dataInput"
            class="search-host"
            @input="setSearch"
      ...
      <div class="selected-user">
        <ul>
          <li v-for="(list , i) in selectedCustomer" :key="i">
            <span>{{ list.name }}</span>
            <button @click="removeCustomerSelect(i)">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                <mask id="mask0_3075_3137" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="24"
                      height="24">
                  <rect width="24" height="24" fill="#D9D9D9"/>
      ...
      <khoonegiButton
          ...
          :disabled="!selectedCustomer.length ? true : false"
      />
    </div>
  </Dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    getMemberList() {
      ...
      this.$store
          .dispatch("getGroupMembersFromTransactions", data)
          .then((respons) => {
            this.isloading = false;
            console.log(respons)
            this.userList = respons.data.customers;
    ...
    removeUser(value) {
      ...
      console.log(data)
      this.$store
          .dispatch("removeGroupMembers", data)
          .then((respons) => {
            this.isloading = false;
            console.log(respons)
            if (respons.data.succeed) {
